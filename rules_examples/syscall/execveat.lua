-- execveat.lua - Hook for execveat syscall
...
local function read_argv(argv_ptr, max_args, max_arg_len)
    ...
    for i = 0, max_args - 1 do
        ...
        local s = select(1, read_cstr(p, max_arg_len))
        if s and s ~= "" then
            table.insert(out, s)
        else
...
function do_syscall(num, dirfd, pathname, argv, envp, flags, arg6, arg7, arg8)
    local path = select(1, read_cstr(pathname, 4096)) or string.format("(unreadable pathname=0x%x)", tonumber(pathname) or 0)
    local args = select(1, read_argv(argv, 64, 512))
    local cmd = join_cmd(args, path)

    if type(c_log) == "function" then
        c_log(string.format("[execveat] %s", cmd))
    end
